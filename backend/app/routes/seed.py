...
def seed_db(db: Session = Depends(get_db)):
    ...
    for cat in CATEGORIES:
        ...
        if not existing:
            cat = Category(id=uuid.uuid4(), name=cat["name"], slug=cat["slug"])
            db.add(cat)
            db.flush()
        cat_ids[cat["slug"]] = existing.id if existing else db.query(Category).filter(Category.slug == cat["slug"]).first().id
    ...
    for prod in PRODUCTS_DATA:
        ...
        cat_id = cat_ids.get(prod["category_slug"])
        product = Product(
            id=uuid.uuid4(),
            name=prod["name"],
            slug=slug,
            description=f"{prod['name']} - สินค้าคุณภาพดี ราคาถูกที่สุด",
        ...
        for price in prod["prices"]:
            p = Price(
                ...
                sold_count=price.get("sold_count", 0),
                affiliate_url=price.get("affiliate_url"),
                product_url=price.get("affiliate_url"),
                created_at=datetime.utcnow(),
            )
            db.add(p)
        created += 1
    ...
    return {"status": "ok", "created": created, "total_products": total_products, "total_prices": total_prices}
